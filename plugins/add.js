...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
	if (!text) throw `_¡Ingresar número!_\nEjemplo:\n\n${usedPrefix + command} ${global.owner[0]}`
	m.reply('_Siendo procesado..._')
    ...
    if (anu[0].attrs.error == 408) conn.sendButton(m.chat, `no se puede agregar a @${anu[0].attrs.jid.split('@')[0]}!\nse dice que @${anu[0].attrs.jid.split('@')[0]} acaba de salir de este grupo `, wm, 'link', usedPrefix + `link`, m)
