...
let handler = async (m, { conn, usedPrefix, owner }) => { 
    try { 
        ...
        if (global.db.data.users[m.sender].health > 79) {
            ...
            let itemrand = [`*Felicidades, tienes un artículo raro que es*\n${mythic} Mythic Crate`,`*Felicidades, tienes un artículo raro que es*\n${legendary} Legendary Crate`]
            let rendem = itemrand[Math.floor(Math.random() * itemrand.length)]
            let str = `
Tu vida se reduce -${health * 1} porque te has aventurado a ${pickRandom(['Jepang', 'Corea', 'Bali', 'America', 'Iraq', 'Arab', 'Pakistan', 'German', 'Finlandia', 'Al mundo de los sueños', 'Ujung dunia', 'Mars', 'Zimbabwe', 'Bulan', 'Pluto', 'Matahari', 'Hatinya dia', '...'])} y obten
*exp:* ${exp} 
*dinero:* ${uang}
*tiketcoin:* 1
...
`.trim()
            ...
            } else conn.reply(m.chat, `Has sido aventurero y estás cansado, por favor inténtalo *${timers}* lagi`, m)
        } else conn.reply(m.chat, 'Mínimo 80 de salud para poder aventurarte, compra una vida primero escribiendo *' + usedPrefix + 'shop buy potion <jumlah>*\ny tipo *' + usedPrefix + 'use potion <jumlah>*', m)
    } catch (e) {
        console.log(e)
